...
from utils.regularization import compute_mrv_loss, compute_hamming_loss, compute_rrv_loss
        ...
        for dataset_name in tqdm(config["datasets"], desc="Dataset Loop"):
            for batch_size in tqdm(config["batch_sizes"], desc="Batch Sizes Loop"):
                ...
                if len(activations["penultimate"]) > 0:
                    ...
                    save_major_regions(major_regions, unique_patterns, dataset_name, batch_size, modelname, logger, prs_enabled=True, rrv_enabled=True, warmup_epochs=config["warmup_epochs"])
                else:
                ...
                for epoch in tqdm(range(warmup_epochs, config["epochs"]), desc=f"PRS Regularized Training {dataset_name} | Batch {batch_size}"):
                    ...
                    if epoch == config["epochs"] - 1:
                        save_model_checkpoint(
                            ...
                            metrics, logger, prs_enabled=True, config=config, extra_tag="RRV"
                        )
