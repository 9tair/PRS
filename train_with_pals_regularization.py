...
from utils.regularization import compute_mrv_loss, compute_hamming_loss, pals_loss
...
def load_checkpoint_if_exists(model, optimizer, modelname,
    ...
    ckpt_dir = os.path.join("models", "saved", f"{modelname}_{dataset_name}_batch_{batch_size}_nan")
    ...
    for modelname in tqdm(config.get("models", []), desc="Model"):
      for dataset_name in tqdm(config.get("datasets", []), desc="Dataset"):
       for batch_size   in tqdm(config.get("batch_sizes", []), desc="Batch size"):
        ...
        criterion = nn.CrossEntropyLoss(label_smoothing=config.get("label_smoothing", 0.2))
        ...
        if major_regions:
            ...
            if major_regions:
                for k, info in major_regions.items():
                    # keys are "class_0", "class_1", …
                    cls_idx = int(k.split('_')[-1])
                    vec = torch.tensor(info["mrv"], dtype=torch.float32,
                                    device=config.get("device", "cpu"))
                    centroids[cls_idx] = vec
                logger.info(f"[PaLS] Cached {len(centroids)} centroids on device "
                            f"{config.get('device','cpu')}.")
                # >>> ADDED ­­­ PaLS-centroid preview
                if config.get("use_pals", False) and centroids:
                    for cls_i, v in centroids.items():
                        nz = v[v != 0]
                        logger.info(
                            f"[PaLS-Centroid][cls={cls_i:2}] len={v.numel():4d} "
                            f"non-zeros={nz.numel():4d} first_nz={nz[:10].tolist()}"
                        )
                # <<< end preview
            else:
                centroids = None
        ...
        metrics_prs = {
            ...
            "mrv_loss": [], "hamming_loss": [], "ce_loss_prs": [], "pals_loss": []
        }
        ...
        for epoch in range(start_prs_epoch, total_ep):
            ...
            if config.get("recompute_mr", True):
                if activations_from_last_full_epoch_for_mrv and labels_from_last_full_epoch_for_mrv:
            ...
            current_epoch_total_ce_loss = 0.0; current_epoch_total_mrv_loss = 0.0; current_epoch_total_ham_loss = 0.0; current_epoch_total_combined_loss = 0.0; current_epoch_total_pals_loss = 0.0
            ...
            for batch_idx, (inp, lbl) in enumerate(train_loader):
                ...
                with torch.amp.autocast(device_type=config.get("device", "cpu").split(':')[0], enabled=use_amp):
                    ...
                    # ------------------------------------------------------------------------------
                    #  BUILD FULL LOSS FOR THIS MINI-BATCH
                    # ------------------------------------------------------------------------------
                    combined_loss_batch = config.get("lambda_ce", 1.0) * ce_loss_batch   # (a) CE

                    pals_loss_batch = torch.tensor(0.0, device=combined_loss_batch.device)
                    if config.get("use_pals", False) and centroids:                      # (b) PaLS
                        pals_loss_batch = pals_loss(
                            activations["current"], lbl, centroids,
                            alpha=config.get("alpha", 0.1)
                        )
                        logger.info(f"[PALS] pals_loss_batch.requires_grad = {pals_loss_batch.requires_grad}")
                        combined_loss_batch += config.get("pals_lambda", 0.5) * pals_loss_batch
                        current_epoch_total_pals_loss += pals_loss_batch.item() * inp.size(0)

                    mrv_loss_batch  = torch.tensor(0.0, device=combined_loss_batch.device)  # (c) MRV & HAM
                    ham_loss_batch  = torch.tensor(0.0, device=combined_loss_batch.device)
                    if major_regions:
                        mrv_loss_batch = compute_mrv_loss(activations["current"], lbl,
                                                        major_regions, logger)
                        ham_loss_batch = compute_hamming_loss(activations["current"], lbl,
                                                            major_regions, logger)
                        combined_loss_batch += (
                    ...
                    logger.debug(                                                          # (d) LOG
                        f"[Loss B{batch_idx+1:3d}] "
                        f"CE={ce_loss_batch.item():.4f}  "
                        f"PaLS={pals_loss_batch.item():.4f}  "
                        f"MRV={mrv_loss_batch.item():.4f}  "
                        f"HAM={ham_loss_batch.item():.4f}"
                    )
                    # ------------------------------------------------------------------------------
                ...
                # place right after combined_loss_batch.backward()
                with torch.no_grad():
                    ce_part   =  config["lambda_ce"]   * ce_loss_batch.item()
                    pals_part =  config["pals_lambda"] * pals_loss_batch.item()
                    mrv_part  =  config["lambda_mrv"]  * mrv_loss_batch.item()
                    ham_part  =  config["lambda_hamming"] * ham_loss_batch.item()
                    logger.info(f"[GradCheck] Loss parts  CE:{ce_part:.3f}  "
                                f"PaLS:{pals_part:.3f}  MRV:{mrv_part:.3f}  HAM:{ham_part:.3f}")

                # and, just to be 100 % sure gradients flow:
                first_w = next(p for n,p in model.named_parameters() if p.requires_grad)
                logger.info(f"[GradCheck] ‖∇first_trainable_param‖₂ = {first_w.grad.norm():.3e}")
            ...
            avg_pals_loss_epoch = current_epoch_total_pals_loss / total_samples_epoch if total_samples_epoch > 0 else 0
            ...
            logger.info(
                f"PRS E{epoch_num_display}/{total_ep} "
                f"CombLoss:{avg_combined_loss_epoch:.4f} "
                f"(CE:{avg_ce_loss_prs_epoch:.4f} "
                f"PaLS:{avg_pals_loss_epoch:.4f} "
                f"MRV:{avg_mrv_loss_epoch:.4f} "
                f"HAM:{avg_ham_loss_epoch:.4f}) "
                f"TrAcc:{train_acc_epoch:.2f}% "
                f"TeAcc:{test_acc_epoch:.2f}% "
                f"PRS:{current_epoch_prs_value:.4f}"
            )
            ...
            metrics_prs["pals_loss"].append(avg_pals_loss_epoch)
            ...
            if epoch_num_display in save_ep:
                ...
                save_model_checkpoint(
                    ...
                    prs_enabled=True, major_regions=major_regions, unique_patterns=unique_patterns_to_save, extra_tag="PALS_2048"
                )
