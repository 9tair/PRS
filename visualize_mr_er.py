...
def extract_params_from_filename(filename):
    """Extracts model name, dataset, and batch size from the filename."""
    match = re.search(r"major_regions_(.+?)_(.+?)_batch_(\d+)", filename)
    
    if not match:
        raise ValueError(f"Invalid filename format: {filename}. Expected format: major_regions_<MODEL>_<DATASET>_batch_<BATCH_SIZE>.json")
    
    model_name, dataset_name, batch_size = match.groups()
    return model_name, dataset_name, int(batch_size)

def load_major_region_data(filename):
    """Loads major region JSON data based on the provided filename."""
    if not os.path.exists(filename):
        raise FileNotFoundError(f"File {filename} not found.")
    
    print(f"🔹 Loading major region data from {filename}...")
    with open(filename, "r") as f:
        data = json.load(f)

    return data

def plot_mr_stacked_bar(filename):
    """Generates a stacked bar plot for major and extra regions per class."""
    
    # Extract parameters from the filename
    model_name, dataset_name, batch_size = extract_params_from_filename(filename)
    
    data = load_major_region_data(filename)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--file", type=str, required=True, help="Filename of the major region JSON (e.g., results/major_regions_CNN-6_CIFAR10_batch_128.json)")
