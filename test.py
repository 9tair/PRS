...
# ---------------------- Step 2: Save Model and Checkpoint ----------------------
def save_model_checkpoint(model, optimizer, dataset_name, batch_size, metrics, logger):
...
# ---------------------- Step 3: NaN Checking for Loss ----------------------
def compute_loss(criterion, outputs, labels, logger):
    ...
    for modelname in tqdm(config['models'], desc="Model Loop"):
        for dataset_name in tqdm(config["datasets"], desc="Dataset Loop"):
            for batch_size in tqdm(config["batch_sizes"], desc="Batch Sizes Loop"):
                ...
                hook_handle = register_activation_hook(model, activations, modelname, dataset_name, batch_size, logger)  # Hook into penultimate layer
                ...
                for epoch in tqdm(range(config["epochs"]), desc=f"Training {dataset_name} | Batch {batch_size}"):
                    ...
                    logger.info(f"Epoch {epoch+1}/{config['epochs']} | Training started.")
                    ...
                    for batch_idx, (inputs, labels) in enumerate(train_loader):
                        ...
                        if loss is None:
                            continue  # Skip batch if NaN detected
                    ...
                    if len(activations["penultimate"]) > 0:
                        ...
                        logger.error(f"No activations collected in epoch {epoch+1}, skipping PRS calculation.")
                        continue

                    # Compute PRS Ratio
                    prs_ratio = compute_unique_activations(epoch_activations[-1], logger) / len(train_dataset)
                    ...
                    logger.info(f"Epoch {epoch+1}/{config['epochs']} | Loss: {epoch_loss/len(train_loader):.4f} | Train Acc: {train_accuracy:.2f}% | Test Acc: {test_accuracy:.2f}% | PRS Ratio: {prs_ratio:.4f}")

                hook_handle.remove()

                # Compute and Save MR/ER/MRV
                if len(epoch_activations) > 0 and len(epoch_labels) > 0:
                    major_regions, unique_patterns = compute_major_regions(np.vstack(epoch_activations), np.hstack(epoch_labels), num_classes=10, logger=logger)
                    
                    # 🔹 Fixed the function call to match the reference code:
                    save_major_regions(major_regions, unique_patterns, dataset_name, batch_size, modelname, logger)
                else:
                    logger.error("No valid activations found for MR computation!")
                ...
                save_model_checkpoint(model, optimizer, dataset_name, batch_size, metrics, logger)

                logger.info(f"Training completed for Model: {modelname}, Dataset: {dataset_name}, Batch Size: {batch_size}")
